...
class SimulatedNetworkEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.links = None
        self.senders = None
        self.create_new_links_and_senders() # important function
        self.net = Network(self.senders, self.links) # makes a function of networks
        self.run_dur = None
        self.run_period = 0.1
        self.steps_taken = 0
    ...
    def step(self, actions):
        ...
        print (event["Latency Inflation"])
        print (event["Latency Ratio"])
        print (event["Send Ratio"])
        print ("\n")
    ...
    """ 
        This function creates a list of senders and links. 
    """
    def create_new_links_and_senders(self):
        bw    = random.uniform(self.min_bw, self.max_bw) # Random bandwidth
        lat   = random.uniform(self.min_lat, self.max_lat) # Random latitude
        queue = 1 + int(np.exp(random.uniform(self.min_queue, self.max_queue))) # Random queue
        loss  = random.uniform(self.min_loss, self.max_loss) # Random loss

        # Random selections from unifrom distribution. 
        #bw    = 200
        #lat   = 0.03
        #queue = 5
        #loss  = 0.00
        ...
        # arguments to sender: 5 arguments
        # uniform distribution of bandwidth; list of self.links first two elements; zero, the features, length of history
...
#print (env.min_bw)#
#print (env.step(np.array([-0.56209713])))
